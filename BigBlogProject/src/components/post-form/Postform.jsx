...
export default function PostForm({post}){
  const {register, handleSubmit, watch,setValue,control, getValues} = useForm({
    ...
    },  //watch is used if u want to continuously monitor the field of form
  }) // the control in here is passed in RTE control
   ...
   React.useEffect(()=>{
        const subscription = watch((value , {name})=>{
            if(name === 'title'){
                setValue('slug', slugTranform(value.title,{//setValue in slug
                    shouldValidate:true
                }))
            }
        })
        ...
        return () => {
            subscription.unsubscribe() // this is use for memory optimization
        }
